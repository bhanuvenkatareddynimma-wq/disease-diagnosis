...
if page=="Home":
    ...
    st.markdown("""<div style="background-color:#FFC107;color:white;font-size:24px;font-weight:bold;padding:12px;border-radius:6px;text-align:center;width:50%;margin:auto;margin-top:15px;">Artificial Intelligence & Data Science</div>""",unsafe_allow_html=True)
...
""",unsafe_allow_html=True)
    ...
    for i in range(0, len(names), 2):
        ...
        for col, name in zip(cols, names[i:i+2]):
            with col:
                st.markdown(f"<div style='background:rgba(30,144,255,0.45);padding:12px;border-radius:8px;text-align:center;color:white;font-weight:600;font-size:16px;white-space:nowrap;'>{name}</div>", unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
